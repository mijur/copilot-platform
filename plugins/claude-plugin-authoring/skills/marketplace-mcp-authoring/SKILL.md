...
- **GitHub Copilot:** use the root `plugin.json` `mcpServers` reference to an in-package `.mcp.json` for MCP-backed capabilities. In a custom-agent profile, target individual tools as `<server>/<tool>` or all server tools as `<server>/*`.
