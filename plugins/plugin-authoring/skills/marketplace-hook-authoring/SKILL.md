...
- **GitHub Copilot:** point the root `plugin.json` `hooks` field at `hooks/hooks.json`. The existing pattern uses `version: 1`, a hook `type: "command"`, platform-specific `bash` and `powershell` commands, `${PLUGIN_ROOT}`, and `timeoutSec`.
